...
"""
 The service to implement network-related interfaces
"""
import logging
import threading
...
def start(args):
    ...
    def setStaticIp(interfaceName, ipAddress, networkPrefixLength, gateway, dns1, dns2):
        ...
        output = run_nmcli_command("nmcli con modify OpenfdeEthernet ifname " + interfaceName + " ipv4.addresses " + ipAddress + "/" + str(networkPrefixLength) + " ipv4.gateway " + gateway + " ipv4.dns '" + dns1 + " " + dns2 + "' ipv4.method 'manual'")
        logging.debug(output)
        if not output:
            logging.debug("nmcli con add:")
            output = run_nmcli_command("nmcli con add con-name OpenfdeEthernet ifname " + interfaceName + "  type ethernet ip4 " + ipAddress + "/" + str(networkPrefixLength) + " gw4 " + gateway + " ipv4.dns '" + dns1 + " " + dns2 + "' ipv4.method 'manual'")
        output = run_nmcli_command("nmcli connection up OpenfdeEthernet")
        if output:
    ...
    def setDHCP(interfaceName):
        ...
        output = run_nmcli_command("nmcli con modify OpenfdeEthernet ifname " + interfaceName + " ipv4.method 'auto'")
        logging.debug(output)
        if not output:
            logging.debug("nmcli con add:")
            output = run_nmcli_command("nmcli con add con-name OpenfdeEthernet ifname " + interfaceName + " ipv4.method 'auto'")
        output = run_nmcli_command("nmcli connection up OpenfdeEthernet")
        if output:
    ...
    def getAllSsid():
        ...
        output = run_nmcli_command("nmcli -t -f ssid dev wifi|sed '/^$/d'")
        logging.debug(output)
        if output == "success":
    ...
    def connectSsid(ssid, passwd):
        ...
        if output:
            ...
            return 1
    def getActivedWifi():
        output = run_nmcli_command("nmcli -f IN-USE,SSID device wifi |grep '*       '|sed -n 1p")
        logging.debug(output)
        if output:
    ...
    def isWifiEnable():
        ...
        if output == "wifi:unavailable":
            logging.debug("isWifiEnable disable")
            return 0
        else:
            logging.debug("isWifiEnable enable")
            return 1
