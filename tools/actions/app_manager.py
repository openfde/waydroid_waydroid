...
def launch(args):
    def justLaunch():
        platformService = IPlatform.get_service(args)
        if platformService:
            #openfde only use single window, no need to update openfde.active_apps
...
def compatbile_get(args):
    def justLaunch():
        ...
        if platformService:
            #openfde only use single window, no need to update waydroid.active_apps 
            ret = platformService.compatbileGet(args.PACKAGE,args.ACTIVITYNAME,args.KEYCODE)
            print("\t" + ret)
        else:
            logging.error("Failed to access IPlatform service")
    maybeLaunchLater(args, justLaunch)   
...
def compatbile_set(args):
    def justLaunch():
        ...
        if platformService:
            #openfde only use single window, no need to update waydroid.active_apps 
            ret = platformService.compatbileSet(args.PACKAGE,args.ACTIVITYNAM,args.KEYCODE,args.VALUE)
            print("set success" )
...
def showFullUI(args):
    def justShow():
        platformService = IPlatform.get_service(args)
        if platformService:
            platformService.setprop("openfde.active_apps", "Openfde")
