...
def generate_nodes_lxc_config(args):
    ...
    make_entry("/dev/fuse")
    if os.path.exists("/dev/dma_heap/system"):
...
def make_base_props(args):
    ...
    if not gralloc:
        if dri:
            ...
            if vulkan == "powervr":
                ...
                elif os.path.exists("/usr/lib/aarch64-linux-gnu/libgbm_ftg.so"):
                    hybris_gbm_file = "/usr/lib/aarch64-linux-gnu/libgbm_ftg.so"
            else :
                if vulkan == "":
                    gralloc = "default"
                    egl = "angle"
    ...
    if gralloc == "FTG340":
        props.append("ro.sf.lcd_density=256")
    else:
